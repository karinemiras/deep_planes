...
path = '/Users/kdo210/Documents/deeplearning/salience_object_detection-master/'
...
for (m in 1:length(methods))
{
    ...
    opt   = read.table(paste(path, methods[m],".txt", sep=''), header = FALSE)
    opt$Model = methods[m] 
...
ggsave(paste( path ,'/opt_train.pdf',  sep=''), graph , device='pdf', height = 8, width = 10)
...
ggsave(paste( path ,'/opt_val.pdf',  sep=''), graph , device='pdf', height = 8, width = 10)
